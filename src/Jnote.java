...
public class Jnote extends JFrame {
	...
	private void initTextArea() {
		...
		JScrollPane pane1=new JScrollPane(textArea, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
	...
	private void initMenuListener() {
		...
		menuAbout.addActionListener(event ->{
			JOptionPane.showOptionDialog(null, "Note 1.0  小宇科技出品", 
					"关于 Note", 
	...
	private void openFile(ActionEvent event){
		if (stareBar.getText().equals("未修改")) {
			...
			int option=JOptionPane.showConfirmDialog(null, "文档已修改，是否存储", "存储文档", 
	...
	private void showfileDialog() {
		...
		if (option==JFileChooser.APPROVE_OPTION) {
			...
			JOptionPane.showMessageDialog(null, e.toString(), "文档打开失败", JOptionPane.ERROR_MESSAGE, null);
		}
		}
	...
	private void closeFile(ActionEvent event){
		if (stareBar.getText().equals("未修改")) {
			...
			int option =JOptionPane.showConfirmDialog(null, "文档已经修改，是否存储", "存储文档", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE, null);
			switch (option) {
			case JOptionPane.YES_OPTION:
				saveFile(event);
				break;
